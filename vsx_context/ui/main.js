...
const DrawButtons = (data) => {
    ...
    for (let i = 0; i < ButtonsData.length; i++) {
        ...
        let id = ButtonsData[i].id
        let element
        element = $(`<div class="button" id=`+id+`><div class="header" id=`+id+`>`+header+`</div><div class="txt" id=`+id+`>`+message+`</div></div>`);
...
const PostData = (id) => {
    $.post(`https://vsx_context/dataPost`, JSON.stringify(Button[id]))
    return CloseMenu();
}

const CancelMenu = () => {
    $.post(`https://vsx_context/cancel`)
    return CloseMenu();
}
